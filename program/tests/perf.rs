...
pub fn setup() -> Mollusk {
    Mollusk::new(
        &solana_bpf_verifier_program::id(),
        "solana_bpf_verifier_program",
    )
}
...
fn test_perf() {
    ...
    let instruction = solana_bpf_verifier_program::instruction::verify(&buffer_address);
